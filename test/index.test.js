...
const getSnapshot = result => result.results.map(({ warnings}) => warnings.map(({ text, ...rest}) => rest));
const getCode = file => fs.readFileSync(path.resolve(__dirname, 'fixtures', file), 'utf-8');
